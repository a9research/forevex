...
| `PIPELINE_POLYGON_RPC_URLS` | **PMA 对齐 indexer** 使用：逗号分隔多个 Polygon JSON-RPC URL（失败/限流会切换 + 退避重试）。**未设置时默认** `https://polygon-rpc.com` 与 `https://rpc.ankr.com/polygon`（公共节点易限流，生产请自备 RPC） |
...
## 如何把新的 raw（Parquet）放进 OSS

权威 raw 在对象存储：`[PIPELINE_S3_PREFIX/]polymarket/blocks/*.parquet`、`…/trades/*.parquet`（可选 `…/markets/*.parquet`）。本地 `PIPELINE_PMA_DATA_DIR` 下的 `polymarket/**` 仅作解压或暂存；**配置了 bucket 时**，`ingest-pma` / `sync` 会先**把本地 parquet 全量上传 OSS 再删除本地目录**，再从 OSS 续跑。

下面三条路任选其一（或组合）：

### 1. 链上增量：Polygon RPC 直接写入 OSS（适合持续追块）

需已配置 **OSS**（`PIPELINE_OSS_*` 或 `PIPELINE_S3_*`）。**`PIPELINE_POLYGON_RPC_URLS` 可不写**：未设置时使用公共 Polygon RPC（易限流，生产请显式配置）。同步 trades 时合约与 topic 有代码内 PMA 默认值，也可在 `.env` 覆盖（见上表与 `.env.example`）。

```bash
cargo run -- indexer-oss              # 默认 blocks + trades，增量写 OSS
cargo run -- indexer-oss --blocks     # 只写 polymarket/blocks/*.parquet + OSS 上 block 游标
cargo run -- indexer-oss --trades     # 只写 polymarket/trades/*.parquet + OSS 上 offset 游标
```

游标文件在 OSS（如 `polymarket/.backfill_block_cursor`、`polymarket/.backfill_offset`），重复执行会从断点继续。可用 `--from-block` / `--to-block` / `--chunk-blocks` / `--blocks-sample-stride` 覆盖默认行为（见 `cargo run -- indexer-oss --help`）。

### 2. 预打包数据：下载解压后自动上传 OSS

```bash
cargo run -- bootstrap-data
```

- 流式下载 **`PIPELINE_BOOTSTRAP_DOWNLOAD_URL`**（默认 `data.tar.zst`）到 `PIPELINE_PMA_DATA_DIR`，解压。
- **若已配置 bucket**：解压后会把本地 **`polymarket/`** 整树上载到 OSS，再删除本地 `polymarket/`（与后续 `ingest-pma` 使用的对象键一致）。
- 已有完成标记不想跳过：加 **`--force`**；调试不上传：加 **`--no-upload`**。

### 3. 本地已有新 Parquet：放入目录后跑管线

将新文件放到 **`$PIPELINE_PMA_DATA_DIR/polymarket/`** 下（例如 `polymarket/trades/…`、`polymarket/blocks/…`），然后：

```bash
cargo run -- ingest-pma    # 或日常一条
cargo run -- sync
```

配置了 OSS 时，会先 **`sync_local_polymarket_to_oss_and_delete`**：上传全部匹配的 `*.parquet` 后再删本地 `polymarket/`，再按 OSS 清单与 **`etl_checkpoint`** 断点续处理。

### 对象键前缀与 markets 说明

- 若设置 **`PIPELINE_S3_PREFIX`**（如 `prod`），则对象键为 **`prod/polymarket/trades/...`** 等形式。
- **`indexer-oss`** 当前面向链上 **blocks / trades**；**`polymarket/markets/*.parquet`** 需来自 bootstrap 包、其它产出流程或自行上传，以便 `PIPELINE_DIM_MARKETS_SOURCE=auto|pma_parquet` 走 OSS markets。

OSS 更新完成后，用 **`cargo run -- status`** 看 trades/blocks/markets 文件数与待处理情况；下游增量仍用 **`cargo run -- sync`**。
...
4. 后续每天/每次向 OSS 补充新的 raw（Parquet）后（拉取方式见上文 **「如何把新的 raw（Parquet）放进 OSS」**）：
```bash
cargo run -- sync
```
